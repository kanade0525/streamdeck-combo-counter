...
Stream Deck アプリの CPU は、キーに絵を送るレートにほぼ比例します（実測）。
...
常駐する以上 12% は選べないので、**絵が変わったときだけ送ります**。残り時間のバーは
20段に量子化してあり、手を止めている間は3秒で約20回。コンボも演出も無くなれば送信を止めます。
60fps に上げるのは段が上がった瞬間の 0.45 秒だけです。
